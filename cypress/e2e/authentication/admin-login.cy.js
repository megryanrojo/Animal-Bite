describe('Admin Login Tests', () => {
  ...
  describe('Login Page UI', () => {
    it('should display the admin login page correctly', () => {
    ...
    it('should display the sidebar with correct information', () => {
      ...
      cy.get('.login-logo').should('contain', 'Animal Bite Admin Portal')
      cy.get('.login-title').should('contain', 'City Health Worker Access')
      cy.get('.login-subtitle').should('contain', 'Log in to access the Animal Bite Incident Admin Portal')
    })

    it('should display security notice', () => {
      cy.get('.security-notice').should('be.visible')
      cy.get('.security-notice').should('contain', 'Security Notice')
      cy.get('.security-notice').should('contain', 'This is a secure admin area')
    })
  ...
  describe('Error Message Display', () => {
    ...
    it('should display error message for invalid email', () => {
      cy.visit('/src/login/admin_login.html?error=invalid_email')
      cy.get('#error-message').should('be.visible')
  ...
  describe('Form Submission', () => {
    it('should submit form with valid credentials', () => {
      ...
      // Should redirect to admin dashboard on successful login
    ...
    it('should set session timeout cookie on form submission', () => {
      cy.get('#email').type(Cypress.env('testAdminEmail'))
      cy.get('#password').type(Cypress.env('testAdminPassword'))
      cy.get('button[type="submit"]').click()
      
      cy.getCookie('session_timeout').should('exist')
    })
  })
