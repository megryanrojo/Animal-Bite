 ...
 * @param string $type Expected data type (string, int, float, date)
...
function sanitizeInput($data, $type = 'string') {
    ...
    switch ($type) {
        ...
        case 'float':
            ...
            return $data !== false ? (float)$data : null;

        case 'date':
            $data = trim($data);
            $date = DateTime::createFromFormat('Y-m-d', $data);
            return $date && $date->format('Y-m-d') === $data ? $data : null;
...
<head>
    ...
    <link rel="stylesheet" href="../css/admin/geomapping.css">
